...
const CircleArea = () => {
  ...
  const generatePairs = gridList => {
    ...
    for (let i = 0; i < totalGrids; i++) {
      ...
      while (!inserted) {
        const pairIdx = Math.floor(Math.random() * Math.floor(pairsRequired))
        if (pairList[pairIdx].pair.length < 2) {
          const newPair = [...pairList[pairIdx].pair, i].sort((a, b) => a - b)
  ...
  const startGame = gridList => {
    ...
    // const gridUpdate = grids.map(obj => {
    //   return {
    //     ...obj,
    //     clicked: true,
    //   }
  ...
  const onCircleClick = pos => {
    ...
    if (selections.length == 2) {
      ...
      const correct = pairs.filter(obj => {
        const { pair } = obj
        if (pair[0] == selections[0] && pair[1] == selections[1]) return true
      })
